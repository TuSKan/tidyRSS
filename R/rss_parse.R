...
rss_parse <- function(doc){
  ...
  if(identical(length(channel), 0L)){
    ...
    res <- suppressWarnings({tibble::tibble(
      ...
      feed_webmaster = xml2::xml_text(xml2::xml_find_first(channel, "webMaster")),
      feed_copyright = xml2::xml_text(xml2::xml_find_first(channel, "copyright")),
      ...
      item_source_name = xml2::xml_text(xml2::xml_find_first(site, "source")),
      item_source_url = xml2::xml_attr(xml2::xml_find_first(site, "source"), "url"),
      item_media_url = xml2::xml_attr(xml2::xml_find_first(site, "media:content"), "url"),
